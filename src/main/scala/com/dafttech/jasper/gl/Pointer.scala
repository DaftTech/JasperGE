...
object Pointer {
  ...
  class VertexPointer(size: Int) extends Pointer {
    override def setup(stride: Int, offset: Int): Unit = glVertexPointer(size, GL_FLOAT, stride, offset)

    override def stride(pointers: Seq[Pointer]): Int = 4 * size
  }

  class ColorPointer(size: Int) extends Pointer {
    override def setup(stride: Int, offset: Int): Unit = glColorPointer(size, GL_FLOAT, stride, offset)

    override def stride(pointers: Seq[Pointer]): Int = 4 * size
  }

  class TexCoordPointer(size: Int) extends Pointer {
    override def setup(stride: Int, offset: Int): Unit = glTexCoordPointer(size, GL_FLOAT, stride, offset)

    override def stride(pointers: Seq[Pointer]): Int = 4 * size
  }

  class NormalPointer extends Pointer {
    override def setup(stride: Int, offset: Int): Unit = glNormalPointer(GL_FLOAT, stride, offset)
